...
class Genome():
    ...
    def genome_to_links(gdicts):
        ...
        for gdict in gdicts:
            ...
            shape_filename = f"{link_name}_shape.obj"
    ...
    @staticmethod
    def to_csv(dna, csv_file, spec):
        # Create the CSV string with headers
        headers = list(spec.keys())
        csv_str = ",".join(headers) + "\n"
        
        # Create a list to hold gene values for each row
        gene_values = [[] for _ in range(len(headers))]
        
        # Populate gene_values with the corresponding DNA values
        for gene in dna:
            for i, (key, val) in enumerate(zip(spec.keys(), gene)):
                gene_values[i].append(str(val))

        # Transpose gene_values to match the CSV structure
        rows = list(zip(*gene_values))
...
class URDFLink:
    ...
    def to_urdf(self, adom):
        link_element = self.to_link_element(adom)
        joint_element = self.to_joint_element(adom)
        return link_element, joint_element

# Example usage within your URDF generation function
def generate_urdf_from_links(links):
    adom = Document()
    robot = adom.createElement("robot")
    robot.setAttribute("name", "genetic_robot")

    for link in links:
        link_element, joint_element = link.to_urdf(adom)
        robot.appendChild(link_element)
        robot.appendChild(joint_element)

    adom.appendChild(robot)
    return adom.toprettyxml()

# # Generate URDF file
# urdf_content = generate_urdf_from_links(links)
# with open("genetic_robot.urdf", "w") as f:
