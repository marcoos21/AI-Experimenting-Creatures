...
rob1 = p.loadURDF('test.urdf', (6,6, 3))


creature_motors = cr.get_motors()

p.setRealTimeSimulation(1)
while p.isConnected():
    for i, motor in enumerate(creature_motors):
        motor_output = motor.get_output()
        p.setJointMotorControl2(
            bodyUniqueId=rob1,
            jointIndex=i,
            controlMode=p.POSITION_CONTROL,
            targetPosition=motor_output
        )
    p.stepSimulation()
    time.sleep(1.0/240)
