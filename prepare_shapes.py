...
# ADVANCED ---------------------------------------------------------------------------------------------------------
def leg_shape_generation(filename, size=5, resolution=0.2):
    vertices = []
    faces = []

    # Generate vertices for a simple leg shape
    for i in range(int(size / resolution)):
        for j in range(int(size / resolution)):
            x = -size/2 + i * resolution
            y = -size/2 + j * resolution
            z = random.uniform(1, 5)  # Height adjustment randomly for variation
            vertices.append([x, y, z])

    # Generate faces for the leg shape
    for i in range(int(size / resolution) - 1):
        for j in range(int(size / resolution) - 1):
            bottom_left = i * int(size / resolution) + j
            bottom_right = i * int(size / resolution) + j + 1
...
# Example usage
leg_shape_generation("leg.obj")



# >80%-------------------------------------------------------------
# Define a function to generate a simple flat terrain
def generate_flat_like_terrain(name_of_file, size=10, resolution=0.5):
    vertices = []
    faces = []

    # Generate vertices for the flat terrain
    for i in range(int(size / resolution)):
        for j in range(int(size / resolution)):
            x = -size/2 + i * resolution
            y = -size/2 + j * resolution
            z = 0  # Flat terrain height
...
# Example usage to generate a flat terrain
generate_flat_terrain("./shapes/flat_terrain.obj")

def generate_pyramid(name_of_file, size=10, resolution=0.5, height=5):
    vertices = []
    faces = []

    # Generate vertices for the top surface of the pyramid
    for i in range(int(size / resolution)):
        for j in range(int(size / resolution)):
            x = -size/2 + i * resolution
            y = -size/2 + j * resolution
            z = height - abs(x) - abs(y)  # Pyramid shape calculation
...
# Example usage to generate a pyramid
generate_pyramid("./shapes/pyramid.obj", size=10, resolution=0.5, height=5)
